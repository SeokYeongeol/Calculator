    ...
    public static void main(String[] args) {
        ...
        while(true) {
            ...
            if(choice.equals("1")) {        // 계산기 실행
                ...
                double value = cal.calculate(val1, val2, operator);
                System.out.println("계산 결과 : " + value + "\n");
                cal.setCalArr(value);              // 배열에 계산한 값 추가
            }


            else if(choice.equals("2")) {   // 계산 데이터 삭제
                if(cal.getCalArr().isEmpty()) {          // 계산 데이터가 없을 때 처음으로 돌아가는 예외 처리문
                    System.out.println("삭제할 컬렉션이 없습니다..\n");
                }
                else {
                    System.out.println("첫 번째 값이 삭제되었습니다! 삭제된 값 : " + cal.getCalArr().get(0) + "\n");
                    cal.delCalArr();
                }
            }
                ...
                if(choiNum.equals("exit")) break;       // 계산기 나가기
                    ...
                    if(cal.getCalArr().isEmpty()) {          // 배열이 비어있을 때 처음으로 돌아가는 예외 처리문
                        System.out.println("조회할 컬렉션이 없습니다. 처음으로 돌아갑니다.\n");
                    }
                    else {          // 배열에 값이 있을 때 출력
                        System.out.print("현재 배열 데이터 : [ ");
                        for(double i : cal.getCalArr()) {
                            if(i != cal.getCalArr().get(cal.getCalArr().size()-1)) System.out.print(i + ", ");
                    ...
                    if(cal.getCalArr().isEmpty()) {
                        ...
                        for(double i : cal.getCalArr()) {        // 입력 값보다 큰 값들을 새 배열에 추가
                            if(i > moreVal) {
                                moreArr.add(i);
                            }
                        }
