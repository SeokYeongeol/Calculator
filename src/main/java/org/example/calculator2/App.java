        ...
        while (true) {
            ...
            if (choice.equals("1")) {       // 계산기 실행
                ...
                try {       // 연산자 입력 예외 처리문
                    ...
                    double addValue = calculate.calculate(firstNumber, secondNumber, operator);
                    System.out.println("계산 결과 : " + addValue + "\n");
                    calculate.setCalculateArr(addValue);
                } catch (IOException e) {
                    System.out.println(e.getMessage());
                }
            } else if (choice.equals("2")) {        // 컬렉션에서 첫 번째 값을 삭제
                if (calculate.getCalculateArr().isEmpty()) {     // 컬렉션이 비어있을 때의 예외처리
                    System.out.println("삭제할 컬렉션이 없습니다." + "\n");
                    continue;
                }
                System.out.println("첫 번째 값을 삭제했습니다. 삭제된 값 : [ " + calculate.getCalculateArr().get(0) + " ]" + "\n");
                calculate.deleteCalculateArr();
            } else if (choice.equals("3")) {        // 컬렉션을 조회
                if (calculate.getCalculateArr().isEmpty()) {
                ...
                if (choiceList.equals("exit")) break;       // 계산기 나가기
                    ...
                    System.out.print("모든 데이터 : " + calculate.getCalculateArr().stream().toList() + "\n");        // calculator 리스트를 조회하는 stream
                    System.out.println();
                    ...
                    try {   // 입력 값보다 높은 데이터를 조회하는 stream + 잘못된 값 입력을 방지하는 예외처리문
                        ...
                        System.out.print(bigValue + " 보다 큰 데이터 : " + calculate.getCalculateArr().stream().filter(cal -> cal > Double.parseDouble(bigValue))
                                .toList() + "\n");
                        System.out.println();
                    } catch (IOException e) {
                        System.out.println(e.getMessage());
